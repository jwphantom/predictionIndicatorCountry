...
def predict_infant_mortality_rate(country_data, country, year):
    subset = country_data[country_data["Country"] == country]
    X = subset["Year"].values
    y = subset["infant deaths"].values
...
def get_infant_mortality_prediction(file_path, country, year):
    data = load_data(file_path)
    return predict_infant_mortality_rate(data, country, year)
